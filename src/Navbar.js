  ...
  return ( 
    <nav className="navbar w-screen flex flex-wrap items-center bg-transparent grid-cols-3 grid-rows-3">
      ...
      <div className="links flex flex-row w-screen">
        ...
        {user?.avatarUrl ? (<img className="rounded-full w-6 h-6 mx-3" alt="avatar" src={user.avatarUrl}/>) : (<svg className="w-6 h-6 bg-transparent mx-3 stroke-current group-hover:stroke-white" fill="none" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5.121 17.804A13.937 13.937 0 0112 16c2.5 0 4.847.655 6.879 1.804M15 10a3 3 0 11-6 0 3 3 0 016 0zm6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>)}
        </button>
        {modal ==='enabled' && <div className="absolute right-0 z-20 w-48 bg-white rounded-md shadow-xl dark:bg-gray-800">
            <div className="p-3">
            <h2>Hello, </h2>
            {error && <div>{error}</div>}
            <strong>{user?.fullName ? user?.fullName : currentUser?.email}</strong></div>
